...
1. Run `meltano install`
1. Feel free to run the pipeline without a mapper `meltano run tap-csv target-sqlite`
1. check your database with `sqlite3 -markdown output/tap_csv.db 'select * from customers limit 20'`
...
1. Look at the data `sqlite3 -markdown output/tap_csv.db 'select * from customers limit 20'`

**Output:**
| email_domain | email_hash | id | \_\_loaded_at |
|------------| --------------------------------| ----------| --------------------------|
...
1. Inspect using `sqlite3 -markdown output/tap_csv.db 'select * from customers_v2 limit 20'`

**Output:**
|description | id | \_\_loaded_at |
|----------- | ---------- | --------------------------|
...
1. Inspect using `sqlite3 -markdown output/tap_csv.db 'select * from customers_v3 limit 20'`

**Output:**
|email | first_name | id | ip_address | last_name | mapping_time | \_\_loaded_at |
|------------------ | ---------- | ---------- |------------- | ---------- | ------------ | --------------------------|
...
1. Run `meltano run tap-csv add_str target-sqlite`
1. Run `sqlite3 -markdown output/tap_csv.db 'PRAGMA table_info(customers_v4)'` to check the type of the "id" column (it should be BIGINT)
1. Inspect using `sqlite3 -markdown output/tap_csv.db 'select * from customers_v4 limit 20'`

**Output:**
|client_id | email | first_name |hash_id | id | ip_address | last_name | \_\_loaded_at |
|---------- | ------------------ | ---------- |-------------------------------- | ----------| ------------- | ---------- | --------------------------|
...
1. Inspect using `sqlite3 -markdown output/tap_csv.db 'select * from customers_v5 limit 20'`
...
1. Inspect using `sqlite3 -markdown output/tap_csv.db 'select * from customers_v6 limit 20'`

**Output:**

|first_name | id |\_\_loaded_at |
|---------- |---------- | --------------------------|
...
1. Inspect using `sqlite3 -markdown output/tap_csv.db 'select * from customers_v7 limit 20'`

**Output:**
|first_name | id | last_name | \_\_loaded_at |
|---------- | ---------- | ---------- | --------------------------|
